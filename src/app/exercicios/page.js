...
export default function Home() {
  return (
    <div className="flex items-center justify-center min-h-screen p-8 pb-20 gap-16">
      <main className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-5 items-center sm:items-start">
        {exercicios.map((num) => {
          ...
          if (fs.existsSync(exercicioPath)) {
            ...
            return (
              <div key={num} className="text-xs sm:text-base">
                <CustomButton destination={`/exercicios/exercicio_${num}`} label={`Exercicio ${formattedNum}`} />
              </div>
            );
          }
          return null;
